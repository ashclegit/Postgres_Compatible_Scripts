...
class ConsoleDisplay(columnHeadings: ArrayList[String],
  ...
  /*private def calcrowSpaceCol(col: Int): Unit = {
    for (i <- 0 until data.size
         if data.get(i).get(col).length > rowSpace.get(col)) {
      rowSpace.set(col, data.get(i).get(col).length)
    }
  }*/
